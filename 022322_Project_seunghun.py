...
@st.cache
def load_data():
    df = pd.read_table("https://github.com/seunghun23/bmi706-final-project-main/blob/master/msk_met_2021_clinical_data.tsv").melt()
    return df
...
clin = pd.read_table("./msk_met_2021_clinical_data.tsv",sep='\t')
...
clin.loc[(clin['Sample Type']=='Primary')&(clin['Metastatic patient']==False),'Sample_Types']="Primary_from_NoMet"
...
with st.sidebar:
    add_radio = st.radio(
        "Choose Panel",
        ("Pan-cancer Clinical information", "Primary vs Metastasis", "Difference in organ sites")
    )
    st.markdown("#")
    st.markdown("#")
...
with st.sidebar:
    st.image("./hms_logo.png",width=200)
...
donut2 = alt.Chart(sampletype_count).mark_arc(innerRadius=50, outerRadius=90).encode(
     theta=alt.Theta(field="patients",aggregate="sum",  type="quantitative"),
    color=alt.Color('Sample_Types:N',scale=type_colorscale,legend=alt.Legend(orient='none',legendX=-130))#,legendY=200))
...
if selected_cancer=="Pan Cancer":
    tmb_dots=alt.Chart(clin).mark_point(size=100).encode(
        ...
        y=alt.Y("median(TMB (nonsynonymous)):Q",title='TMB'),
...
row2=alt.hconcat(donut1, cancer_bar, spacing=27)
